...
int FindSmall(vector<int>Data) //Find two points which have smallest distance between each other
{
	int Check, number, n;
	Check = Data[0];
	n = number = 0;
	for (n = 0; n < Data.size(); n++) //Find which cases the two lines suit
	{
		if (Check>Data[n])
		{
			Check = Data[n];
...
int main()
{
	...
	frame = imread("E:/FrameDetect.jpg", CV_LOAD_IMAGE_COLOR);// Open image from computer in color
	...
	while (!stop)
	{
		...
		imshow("Gray", gray);
		GaussianBlur(gray, blurred, Size(75, 75), 35); //Gaussiban Blur process, large size of radius are used on there.
		imwrite("Blur1.jpg", blurred);
		imshow("Blur", blurred);
		threshold(blurred, thres, 110, 255, THRESH_BINARY); //Thresholding process
		imwrite("thres1.jpg", thres);
		...
		cout << "The smallest distance is " << FindSmall(DistanceShort) << endl; //Put calculated results to function FindSmall

		//
		vector<Vec4i>OppositePoints = lines; 
		switch (FindSmall(DistanceShort)) //Use smallest distance to find correct cases
		{
		case 0:
			break;
		case 1:
		...
		//
		double angle[2], value[3], angle4;//Define variables to store calculation results
